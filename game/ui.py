...
class TerminalUI:
    ...
    def create_status_bar(self) -> Panel:
        ...
        status_text = (
            f"[bold green]HP:[/bold green] {player_info.get('hp', 0)}/{player_info.get('max_hp', 0)} | "
            f"[bold blue]Level:[/bold blue] {player_info.get('level', 1)} | "
            f"[bold yellow]Gold:[/bold yellow] {player_info.get('gold', 0)} | "
            f"[bold magenta]Location:[/bold magenta] {self.game_data.get('location', 'Unknown')}"
        )
    ...
    def create_character_creation(self) -> Layout:
        ...
        classes = [
            ...
            "Warlock", "Druid", "Monk", "Barbarian", "Bard", "Sorcerer",
            "Fighter", "Necromancer", "Illusionist", "Alchemist", 
            "Berserker", "Assassin", "Healer", "Summoner", 
    ...
    def create_character_info_panel(self) -> Panel:
        ...
        info_table.add_row(f"Level {player.get('level', 1)} {player.get('class', 'Warrior')}")
    ...
    def create_inventory_screen(self) -> Layout:
        ...
        if inventory:
            ...
            for i, item in enumerate(inventory):
                item_style = "bold yellow" if i == self.state.selected_index else "white"
        ...
        # Footer
        footer_text = "[dim]↑↓ Select | Enter Use/Equip | Drop Key | Backspace Back[/dim]"
        layout["footer"].update(Panel(footer_text, border_style="dim"))
    ...
    def create_save_load_screen(self, screen_type: str = "save") -> Layout:
        ...
        if saves:
            ...
            for i, save in enumerate(saves):
                save_style = "bold yellow" if i == self.state.selected_index else "white"
                saves_table.add_row(
    ...
    def create_combat_screen(self) -> Layout:
        ...
        header_table.add_row(f"HP: {player.get('hp', 100)}/{player.get('max_hp', 100)}", f"HP: {enemy.get('hp', 50)}/{enemy.get('max_hp', 50)}")
        header_table.add_row(f"Round {combat_data.get('round', 1)}", f"Status: {combat_data.get('status', 'Active')}")
        ...
        layout["footer"].update(Panel(footer_content, title="Actions", border_style="cyan"))
    ...
    def handle_main_menu_input(self, key: str) -> None:
        ...
        if key in ["up", "k"]:
            self.state.selected_index = (self.state.selected_index - 1) % len(menu_items)
        elif key in ["down", "j"]:
            self.state.selected_index = (self.state.selected_index + 1) % len(menu_items)
        elif key == "enter":
    ...
    def handle_inventory_input(self, key: str) -> None:
        ...
        if key in ["up", "k"]:
            ...
            self.state.selected_index = min(len(inventory) - 1, self.state.selected_index + 1)
        elif key == "enter":
    ...
    def handle_save_load_input(self, key: str) -> None:
        ...
        if key in ["up", "k"]:
            ...
            self.state.selected_index = min(len(saves) - 1, self.state.selected_index + 1)
        elif key == "enter":
    ...
    def handle_combat_input(self, key: str) -> None:
        ...
        if key in ["up", "k"]:
            ...
            self.state.selected_index = min(len(actions) - 1, self.state.selected_index + 1)
        elif key == "enter":
