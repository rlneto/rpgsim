...
def _get_stats_obj(character_or_enemy: Union[Character, Enemy]) -> Any:
    """
    Helper to get stats object that works with both dict and CharacterStats.

    Args:
        character_or_enemy: Character or Enemy to get stats from
...
def calculate_damage(
    ...
    # Get base stats
    attacker_stats = _get_stats_obj(attacker)
    _ = _get_stats_obj(defender)  # Unused but needed for validation
    ...
    if is_critical:
        critical_multiplier = CRITICAL_MULTIPLIERS.get("critical", 2.0)
...
def calculate_hit_chance(
    ...
    # Get stats
    attacker_stats = _get_stats_obj(attacker)
    defender_stats = _get_stats_obj(defender)
...
def calculate_dodge_chance(
    ...
    Examples:
        ...
        True
    """
    # Get stats - handle both dict and CharacterStats objects
    defender_stats = _get_stats_obj(defender)
    attacker_stats = _get_stats_obj(attacker)
...
def resolve_combat(attacker: Union[Character, Enemy], defender: Union[Character, Enemy]) -> Dict[str, Any]:
    ...
    Args:
        attacker: Character or Enemy initiating the attack
        defender: Character or Enemy defending

    Returns:
        Dict with combat results including winner, damage dealt, and combat log
    """
    # Use existing combat system to resolve the fight
    outcome = calculate_combat_outcome([attacker, defender])
