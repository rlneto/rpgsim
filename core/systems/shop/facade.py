...
from .services.shop_service import (
    ShopCreationService, ShopTransactionService, ShopInventoryService,
    ShopEconomyService, ShopManagementService
)
from .services.item_service import ItemGenerationService, ItemPricingService
from .repositories.memory_repository import (
    MemoryShopRepository, MemoryShopItemRepository, MemoryShopTransactionRepository
)
...
class ShopSystem:
    ...
    def __init__(self, shop_repository=None, item_repository=None, transaction_repository=None):
        ...
        self.transaction_repo = transaction_repository or MemoryShopTransactionRepository(self.shop_repo)
    ...
    def create_shop(self, shop_id: str, name: str, shop_type: str, owner: str,
                    location: str, quality_level: str = ShopQuality.STANDARD.value,
    ...
    # Transaction Methods
    def process_transaction(self, shop_id: str, transaction_type: str, 
                           item_id: Optional[str] = None, item: Optional[ShopItem] = None,
                           character_gold: int = 0, character_reputation: int = 50) -> Dict[str, Any]:
        ...
        if transaction_type == "buy":
            if not item_id:
                return {"success": False, "reason": "Item ID required for buy transaction"}
            result = self.transaction_service.process_buy_transaction(
                shop, item_id, character_gold, character_reputation
            )
        elif transaction_type == "sell":
            if not item:
                return {"success": False, "reason": "Item required for sell transaction"}
    ...
    def calculate_buy_price(self, shop_id: str, item_id: str, 
                            character_reputation: int = 50) -> Optional[int]:
        ...
        return self.pricing_service.calculate_buy_price(shop, item, character_reputation)
    
    def calculate_sell_price(self, shop_id: str, item: ShopItem, 
                              character_reputation: int = 50) -> Optional[int]:
        ...
        return self.pricing_service.calculate_sell_price(shop, item, character_reputation)
    ...
    def restock_shop(self, shop_id: str, items: Optional[List[ShopItem]] = None) -> Dict[str, Any]:
    ...
    # Economic Methods
    def update_shop_economy(self, shop_id: str, economic_change: int, 
        ...
        result = self.economy_service.update_shop_economy(shop, economic_change, location_wealth_change)
        self.shop_repo.save_shop(shop)
        return result
    ...
    def get_transaction_history(self, shop_id: str, limit: int = 100) -> List[ShopTransaction]:
    ...
    def calculate_dynamic_price(self, shop_id: str, item_id: str,
                                supply_factor: float = 1.0, demand_factor: float = 1.0,
                                location_wealth: float = 1.0) -> Optional[int]:
    ...
    def calculate_bulk_purchase_price(self, shop_id: str, item_ids: List[str],
                                       character_reputation: int = 50) -> Optional[Dict[str, Any]]:
    ...
    def refresh_shop_inventory(self, shop_id: str, current_day: int = 1) -> Dict[str, Any]:
...
def create_shop(shop_id: str, name: str, shop_type: str, owner: str,
                 location: str, quality_level: str = ShopQuality.STANDARD.value) -> Optional[Shop]:
    """Create shop (backward compatibility)"""
    return _shop_system.create_shop(shop_id, name, shop_type, owner, location, quality_level)


def buy_item_from_shop(shop_id: str, item_id: str, character_gold: int, 
                        character_reputation: int = 50) -> Dict[str, Any]:
    """Buy item from shop (backward compatibility)"""
    return _shop_system.process_transaction(
        shop_id, "buy", item_id=item_id, character_gold=character_gold,
        character_reputation=character_reputation
    )


def sell_item_to_shop(shop_id: str, item: ShopItem, character_reputation: int = 50) -> Dict[str, Any]:
...
def restock_shop(shop_id: str, items: Optional[List[ShopItem]] = None) -> Dict[str, Any]:
    """Restock shop (backward compatibility)"""
    return _shop_system.restock_shop(shop_id, items)


def calculate_shop_prices(shop_id: str, base_prices: List[int], 
                           character_reputation: int = 50) -> List[int]:
    ...
    return [
        int(price * shop.economy.price_modifier * (1.0 - shop.get_reputation_discount(character_reputation)))
        for price in base_prices
    ]


def update_shop_economy(shop_id: str, economic_change: int, 
                        location_wealth_change: int = 0) -> Dict[str, Any]:
    """Update shop economy (backward compatibility)"""
    return _shop_system.update_shop_economy(shop_id, economic_change, location_wealth_change)
...
__all__ = [
    'ShopSystem',
    'create_shop', 'buy_item_from_shop', 'sell_item_to_shop',
    'get_shop_inventory', 'restock_shop', 'calculate_shop_prices',
    'update_shop_economy', 'generate_shop_inventory', 'get_shop_data'
]
