...
Comprehensive travel mechanics with time costs, random events, resource consumption, and fast travel
Includes encounter systems, terrain effects, equipment bonuses, and progression-based unlocks
"""
...
class TravelSystem:
    ...
    TRAVEL_METHODS = {
        ...
        TravelMethod.BOAT: {
            "time_multiplier": 0.6, "cost_multiplier": 2.0, "comfort": 2,
            "terrain_specific": [TerrainType.COASTAL]
        },
        TravelMethod.SHIP: {
            "time_multiplier": 0.4, "cost_multiplier": 3.0, "comfort": 4,
            "terrain_specific": [TerrainType.COASTAL]
        },
        TravelMethod.TELEPORT: {
            "time_multiplier": 0.1, "cost_multiplier": 10.0, "comfort": 5,
    ...
    EQUIPMENT_BONUSES = {
        ...
        TravelEquipment.CART: {
            "time_reduction": 0.2, "cost_reduction": 0.1, "comfort_bonus": 2,
            "party_bonus": 2
        },
        TravelEquipment.BOAT: {
            "time_reduction": 0.4, "cost_reduction": 0.3, "comfort_bonus": 1,
            "terrain_specific": [TerrainType.COASTAL]
        },
        TravelEquipment.CLIMBING_GEAR: {
            "time_reduction": 0.25, "cost_reduction": 0.15, "comfort_bonus": 1,
            "terrain_specific": [TerrainType.MOUNTAINS]
        },
        TravelEquipment.COMPASS: {
            "time_reduction": 0.1, "cost_reduction": 0.05, "navigation_bonus": 1
        },
        TravelEquipment.LANTERN: {
            "time_reduction": 0.0, "cost_reduction": 0.0, "night_travel": 1
        },
        TravelEquipment.CLOAK: {
            "time_reduction": 0.0, "cost_reduction": 0.0, "weather_protection": 1
        },
        TravelEquipment.SURVIVAL_KIT: {
            "time_reduction": 0.0, "cost_reduction": 0.25, "resource_efficiency": 1
        }
    }
    ...
    def calculate_encounter_chance(self, route: TravelRoute, plan: TravelPlan, character_level: int) -> float:
        ...
        final_chance = base_chance * distance_modifier * danger_modifier * terrain_modifier
        final_chance *= (1 - level_reduction) * party_modifier * safe_modifier

        return min(0.9, final_chance)  # Cap at 90%

    def generate_travel_events(self, route: TravelRoute, plan: TravelPlan, character_level: int) -> List[TravelEventData]:
        ...
        for _ in range(num_events):
            # Select event type
            event_types = [
                TravelEvent.MERCHANT_ENCOUNTER, TravelEvent.BANDIT_ATTACK,
                TravelEvent.WEATHER_CHANGE, TravelEvent.WILD_ANIMAL,
                TravelEvent.LOST_TRAVELER, TravelEvent.ANCIENT_RUINS,
                TravelEvent.NATURAL_RESOURCE, TravelEvent.ROAD_BLOCKAGE,
                TravelEvent.FRIENDLY_PATROL, TravelEvent.MYSTERIOUS_STRANGER,
                TravelEvent.TREASURE_MAP, TravelEvent.EQUIPMENT_FAILURE
            ]
            ...
            choices = self._generate_event_choices(event_type) if event_type in [
                TravelEvent.LOST_TRAVELER, TravelEvent.ANCIENT_RUINS,
                TravelEvent.ROAD_BLOCKAGE, TravelEvent.MYSTERIOUS_STRANGER
            ] else []

            events.append(TravelEventData(
    ...
    def _generate_event_details(self, event_type: TravelEvent, terrain: TerrainType, character_level: int) -> Dict[str, Any]:
        ...
        if event_type == TravelEvent.MERCHANT_ENCOUNTER:
            ...
            details = {
                "animal_type": random.choice(["wolf", "bear", "boar", "eagle", "snake"]),
            ...
            weather_types = {
                TerrainType.MOUNTAINS: ["snow_storm", "avalanche_risk", "clear_skies"],
                TerrainType.DESERT: ["sand_storm", "extreme_heat", "cool_night"],
                TerrainType.SWAMP: ["heavy_rain", "fog", "mosquito_swarm"],
                TerrainType.COASTAL: ["storm", "high_tides", "calm_seas"]
            }
            details = {
                "weather_type": random.choice(weather_types.get(terrain, ["rain", "wind", "clear"])),
            ...
            details = {
                "traveler_type": random.choice(["merchant", "pilgrim", "scholar", "noble"]),
                "condition": random.choice(["injured", "lost", "sick", "robbed"]),
                "needs_help": True,
                "potential_reward": random.choice([False, True, True])  # Higher chance of reward
            }
        elif event_type == TravelEvent.ANCIENT_RUINS:
            details = {
                "ruin_type": random.choice(["temple", "fortress", "tower", "village"]),
                "condition": random.choice(["partially_intact", "overgrown", "collapsing"]),
            ...
            details = {
                "resource_type": random.choice(resource_types.get(terrain, ["herbs", "water"])),
            ...
            details = {
                "blockage_type": random.choice(["landslide", "flooded_path", "fallen_trees", "bandit_ambush"]),
    ...
    def can_fast_travel(self, player_id: str, _from_location: str, to_location: str, character_level: int) -> Tuple[bool, str]:
    ...
    def update_travel(self, player_id: str, hours_passed: int) -> Optional[ActiveTravel]:
        ...
        # Check for events at current time (events are available for game systems to process)
    ...
    def discover_route(self, player_id: str, route: TravelRoute) -> None:
        ...
        existing = [
            r for r in self.discovered_routes[player_id]
            if r.from_location == route.from_location and r.to_location == route.to_location
        ]
    ...
    def get_available_destinations(self, player_id: str, current_location: str, character_level: int) -> List[TravelRoute]:
        ...
        return [
            route for route in player_routes
            if route.from_location == current_location and
               not route.restricted and
               route.level_requirement <= character_level
        ]

    def calculate_fast_travel_cost(self, route: TravelRoute, character_level: int) -> TravelCost:
...
def get_all_terrain_types() -> List[str]:
    ...
    return [
        TerrainType.PLAINS, TerrainType.FOREST, TerrainType.MOUNTAINS,
        TerrainType.HILLS, TerrainType.SWAMP, TerrainType.DESERT,
        TerrainType.COASTAL, TerrainType.TUNDRA, TerrainType.JUNGLE, TerrainType.VOLCANIC
    ]
...
def get_all_travel_methods() -> List[str]:
    ...
    return [
        TravelMethod.WALK, TravelMethod.HORSE, TravelMethod.CART,
        TravelMethod.BOAT, TravelMethod.SHIP, TravelMethod.TELEPORT
    ]
...
def get_all_travel_equipment() -> List[str]:
    ...
    return [
        TravelEquipment.MOUNT, TravelEquipment.CART, TravelEquipment.BOAT,
        TravelEquipment.CLIMBING_GEAR, TravelEquipment.COMPASS,
        TravelEquipment.LANTERN, TravelEquipment.CLOAK, TravelEquipment.SURVIVAL_KIT
    ]
...
def validate_travel_requirements(route: TravelRoute, character_level: int, equipment: List[TravelEquipment]) -> bool:
    ...
    terrain_requirements = {
        ...
        TerrainType.VOLCANIC: [TravelEquipment.CLIMBING_GEAR, TravelEquipment.SURVIVAL_KIT]
    }
