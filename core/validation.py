...
def validate_character_name(name: str) -> bool:
    ...
    if not name:
        raise ValidationError(
            ERROR_MESSAGES["character_name_empty"], field="name", value=name
        )

    # Check if name is only whitespace
    if not name.strip():
        raise ValidationError(
            ERROR_MESSAGES["character_name_leading_trailing_spaces"],
            field="name",
            value=name,
        )
    ...
    if len(name.strip()) < VALIDATION_CONFIG["min_character_name_length"]:
        raise ValidationError(
            "Character name must be at least 1 character long", field="name", value=name
        )

    if len(name) > VALIDATION_CONFIG["max_character_name_length"]:
        raise ValidationError(
            ERROR_MESSAGES["character_name_too_long"], field="name", value=name
        )
    ...
    if "  " in name:
        raise ValidationError(
            ERROR_MESSAGES["character_name_double_spaces"], field="name", value=name
        )

    # Check for invalid characters
    if not re.match(f"^[{VALIDATION_CONFIG['valid_name_characters']}]*$", name):
        raise ValidationError(
            ERROR_MESSAGES["character_name_invalid_chars"], field="name", value=name
        )
...
def validate_character_stats(stats: CharacterStats) -> bool:
    ...
    Examples:
        ...
        ...     strength=0, dexterity=10, intelligence=10,
        ...     wisdom=10, charisma=10, constitution=10
        ... )
    ...
    # Validate strength
    if (
        stats.strength < GAME_CONFIG["min_stat_value"]
        or stats.strength > GAME_CONFIG["max_stat_value"]
    ):
        raise ValidationError(
            f"Strength must be between {GAME_CONFIG['min_stat_value']} "
            f"and {GAME_CONFIG['max_stat_value']}",
            field="strength",
    ...
    # Validate dexterity
    if (
        stats.dexterity < GAME_CONFIG["min_stat_value"]
        or stats.dexterity > GAME_CONFIG["max_stat_value"]
    ):
        raise ValidationError(
            f"Dexterity must be between {GAME_CONFIG['min_stat_value']} "
            f"and {GAME_CONFIG['max_stat_value']}",
            field="dexterity",
    ...
    if (
        stats.intelligence < GAME_CONFIG["min_stat_value"]
        or stats.intelligence > GAME_CONFIG["max_stat_value"]
    ):
        raise ValidationError(
            f"Intelligence must be between {GAME_CONFIG['min_stat_value']} "
            f"and {GAME_CONFIG['max_stat_value']}",
            field="intelligence",
    ...
    # Validate wisdom
    if (
        stats.wisdom < GAME_CONFIG["min_stat_value"]
        or stats.wisdom > GAME_CONFIG["max_stat_value"]
    ):
        raise ValidationError(
            f"Wisdom must be between {GAME_CONFIG['min_stat_value']} "
            f"and {GAME_CONFIG['max_stat_value']}",
            field="wisdom",
            value=stats.wisdom,
        )
    ...
    # Validate charisma
    if (
        stats.charisma < GAME_CONFIG["min_stat_value"]
        or stats.charisma > GAME_CONFIG["max_stat_value"]
    ):
        raise ValidationError(
            f"Charisma must be between {GAME_CONFIG['min_stat_value']} "
            f"and {GAME_CONFIG['max_stat_value']}",
            field="charisma",
    ...
    if (
        stats.constitution < GAME_CONFIG["min_stat_value"]
        or stats.constitution > GAME_CONFIG["max_stat_value"]
    ):
        raise ValidationError(
            f"Constitution must be between {GAME_CONFIG['min_stat_value']} "
            f"and {GAME_CONFIG['max_stat_value']}",
            field="constitution",
...
def validate_character_creation(
    name: str, class_type: CharacterClass, stats: CharacterStats
) -> bool:
    """
    ...
    if class_type not in CharacterClass:
        raise ValidationError(
            ERROR_MESSAGES["character_class_invalid"],
            field="class_type",
            value=class_type,
        )
...
def validate_class_stat_requirements(
    class_type: CharacterClass, stats: CharacterStats
) -> bool:
    """
    ...
    class_requirements = {
        CharacterClass.WARRIOR: {"strength": 12, "constitution": 12},
        CharacterClass.MAGE: {"intelligence": 12, "wisdom": 10},
        CharacterClass.ROGUE: {"dexterity": 12, "strength": 8},
        CharacterClass.CLERIC: {"wisdom": 12, "charisma": 10},
        CharacterClass.RANGER: {"dexterity": 12, "wisdom": 10},
        CharacterClass.PALADIN: {"strength": 12, "charisma": 12, "wisdom": 10},
    ...
    if class_type in class_requirements:
        ...
        for stat_name, min_value in requirements.items():
            ...
            if stat_value < min_value:
                raise ValidationError(
                    f"{class_type.value.title()} class requires {stat_name} >= {min_value}, "
                    f"got {stat_value}",
                    field=stat_name,
                    value=stat_value,
                )
...
def validate_item_creation(name: str, item_type: ItemType, value: int) -> bool:
    ...
    if len(name) > VALIDATION_CONFIG["max_item_name_length"]:
        raise ValidationError(
            ERROR_MESSAGES["item_name_too_long"], field="name", value=name
        )
    ...
    if value < 0:
        raise ValidationError(
            "Item value cannot be negative", field="value", value=value
        )

    if value > 1000000:  # Reasonable upper limit
        raise ValidationWarning(
            "Item value is unusually high", field="value", value=value
        )

    return True


def validate_enemy_creation(
    name: str, enemy_type: EnemyType, level: int, hp: int, max_hp: int
) -> bool:
    """
    ...
    if level < 1 or level > GAME_CONFIG["max_character_level"]:
        raise ValidationError(
            f"Enemy level must be between 1 and {GAME_CONFIG['max_character_level']}",
            field="level",
            value=level,
        )
    ...
    if max_hp <= 0:
        raise ValidationError(
            "Enemy max HP must be positive", field="max_hp", value=max_hp
        )
...
def validate_quest_creation(
    name: str, description: str, objectives: List[QuestObjective]
) -> bool:
    """
    ...
    if len(name) > VALIDATION_CONFIG["max_quest_name_length"]:
        raise ValidationError(
            ERROR_MESSAGES["quest_name_too_long"], field="name", value=name
        )

    # Validate quest description
    if not description or not description.strip():
        raise ValidationError(
            "Quest description cannot be empty", field="description", value=description
        )

    if len(description) > VALIDATION_CONFIG["max_quest_description_length"]:
        raise ValidationError(
            ERROR_MESSAGES["quest_description_too_long"],
            field="description",
            value=description,
        )
    ...
    for i, objective in enumerate(objectives):
        if not objective.description or not objective.description.strip():
            raise ValidationError(
                f"Objective {i + 1} description cannot be empty",
                field=f"objectives[{i}].description",
                value=objective.description,
            )

        if objective.target < 0:
            raise ValidationError(
                f"Objective {i + 1} target cannot be negative",
                field=f"objectives[{i}].target",
                value=objective.target,
            )

        if objective.progress < 0:
            raise ValidationError(
                f"Objective {i + 1} progress cannot be negative",
                field=f"objectives[{i}].progress",
                value=objective.progress,
            )

        if objective.progress > objective.target:
            raise ValidationError(
                f"Objective {i + 1} progress cannot exceed target",
                field=f"objectives[{i}].progress",
                value=objective.progress,
            )
...
def validate_combat_execution(
    attacker: Character, defender: Union[Character, Enemy], damage: int
) -> bool:
    """
    ...
    if not attacker:
        raise ValidationError(
            "Attacker cannot be None", field="attacker", value=attacker
        )

    if not attacker.is_alive():
        raise ValidationError(
            "Attacker must be alive", field="attacker", value=str(attacker)
        )

    # Validate defender
    if not defender:
        raise ValidationError(
            "Defender cannot be None", field="defender", value=defender
        )

    if not defender.is_alive():
        raise ValidationError(
            "Defender must be alive", field="defender", value=str(defender)
        )

    # Validate damage
    if damage < 0:
        raise ValidationError(
            ERROR_MESSAGES["damage_negative"], field="damage", value=damage
        )

    if damage > GAME_CONFIG["combat_damage_cap"]:
        raise ValidationError(
            ERROR_MESSAGES["damage_exceeds_cap"], field="damage", value=damage
        )
...
def validate_damage_calculation(
    attacker: Character, _defender: Union[Character, Enemy], damage: int
) -> bool:
    """
    ...
    Args:
        attacker: Attacking character
        _defender: Defending character or enemy (unused in current validation)
        damage: Damage dealt
    ...
    if hasattr(attacker, "class_type") and attacker.class_type:
        ...
        attacker_type = (
            attacker.type.value
            if hasattr(attacker.type, "value")
            else str(attacker.type)
        )
    ...
    if damage < min_expected:
        raise ValidationWarning(
            f"Damage {damage} is below expected range {min_expected:.1f}-{max_expected:.1f}",
            field="damage",
            value=damage,
        )
    ...
    if damage > max_expected:
        raise ValidationWarning(
            f"Damage {damage} is above expected range {min_expected:.1f}-{max_expected:.1f}",
            field="damage",
            value=damage,
        )
...
def validate_save_load_operation(
    game_state: GameState, save_data: Optional[str] = None
) -> bool:
    """
    ...
    if not game_state:
        raise ValidationError(
            "Game state cannot be None", field="game_state", value=game_state
        )
    ...
    if save_data is None:
        ...
        if not hasattr(game_state, "save_version"):
            raise ValidationError(
                "Game state missing save_version", field="game_state", value=game_state
            )

        if not hasattr(game_state, "save_timestamp"):
            raise ValidationError(
                "Game state missing save_timestamp",
                field="game_state",
                value=game_state,
            )
    ...
    # For load operations
        ...
        if not isinstance(save_data, str):
            raise ValidationError(
                ...
                field="save_data",
                value=str(type(save_data)),
            )

        if not save_data.strip():
            raise ValidationError(
                "Save data cannot be empty", field="save_data", value=save_data
            )
        ...
        try:
            ...
            raise ValidationError(
                f"Save data is not valid JSON: {str(e)}",
                field="save_data",
                value=save_data,
            ) from e
...
def validate_character_level(character: Character, level: int) -> bool:
    ...
    if not character:
        raise ValidationError(
            "Character cannot be None", field="character", value=character
        )

    if (
        level < GAME_CONFIG["min_character_level"]
        or level > GAME_CONFIG["max_character_level"]
    ):
        raise ValidationError(
            f"Level must be between {GAME_CONFIG['min_character_level']} "
            f"and {GAME_CONFIG['max_character_level']}",
            field="level",
            value=level,
        )
    ...
    if level < character.level:
        raise ValidationError(
            f"Target level {level} cannot be less than current level {character.level}",
            field="level",
            value=level,
        )

    # Validate experience requirement for target level
    # Simple inline calculation to avoid circular import
    if level == 1:
        required_exp = 0
    elif level <= 20:
        required_exp = 100 * (level - 1) ** 2
    else:
        required_exp = int(100 * pow(level - 1, 1.8))

    if character.experience < required_exp:
        raise ValidationError(
            f"Insufficient experience for level {level}: need {required_exp}, "
            f"have {character.experience}",
            field="experience",
...
def validate_ability_learning(
    character: Character, ability: str, required_level: int
) -> bool:
    """
    ...
    if not ability or not ability.strip():
        raise ValidationError(
            "Ability name cannot be empty", field="ability", value=ability
        )

    if character.level < required_level:
        raise ValidationError(
            f"Character level {character.level} insufficient for ability '{ability}' "
    ...
    if character.has_ability(ability):
        raise ValidationError(
            f"Character already knows ability '{ability}'",
            field="ability",
            value=ability,
        )
...
def validate_item_equipment(character: Character, item: Item) -> bool:
    ...
    if not item.equippable:
        raise ValidationError(
            f"Item '{item.name}' is not equippable", field="item", value=item
        )

    if not item.is_equipment():
        raise ValidationError(
            f"Item '{item.name}' is not equipment", field="item", value=item
        )
    ...
    for stat_name, required_value in item.stats_mod.items():
        if required_value > 0:  # Only check positive requirements
            ...
            if character_stat < required_value:
                raise ValidationError(
                    f"Item requires {stat_name} >= {required_value}, "
                    f"character has {character_stat}",
                    field=stat_name,
                    value=character_stat,
                )

    return True


def validate_character_gold_transaction(
    character: Character, amount: int, transaction_type: str
) -> bool:
    """
    ...
    if amount < 0:
        raise ValidationError(
            "Transaction amount cannot be negative", field="amount", value=amount
        )
