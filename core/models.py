...
class CharacterStats(BaseModel):
    ...
    strength: int = Field(default=10, ge=1, le=20, description="Physical strength")
    dexterity: int = Field(default=10, ge=1, le=20, description="Agility and reflexes")
    intelligence: int = Field(default=10, ge=1, le=20, description="Mental capacity")
    wisdom: int = Field(default=10, ge=1, le=20, description="Perception and insight")
    charisma: int = Field(default=10, ge=1, le=20, description="Social influence")
    ...
    def get_stat_modifiers(self) -> Dict[str, int]:
        ...
        modifiers = {}
        for stat_name, stat_value in self.model_dump().items():
...
class Character(BaseModel):
    ...
    def validate_hp_relationships(self):
        """Validate HP relationships after all fields are set."""
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        class_value = (
            ...
            if hasattr(self.class_type, "value")
...
class Item(BaseModel):
    ...
    stats_mod: Dict[str, int] = Field(
        default_factory=dict, description="Stat modifiers"
    )
...
class Quest(BaseModel):
    ...
    description: str = Field(
        min_length=1, max_length=1000, description="Quest description"
    )
    ...
    time_limit: Optional[int] = Field(
        default=None, ge=0, description="Time limit in minutes"
    )
    prerequisites: List[str] = Field(
        default_factory=list, description="Quest prerequisites"
    )
...
class Location(BaseModel):
    ...
    description: str = Field(
        default="", max_length=500, description="Location description"
    )
    ...
    connections: List[str] = Field(
        default_factory=list, description="Connected locations"
    )
...
class GameState(BaseModel):
    ...
    locations: Dict[str, Any] = Field(
        default_factory=dict, description="Discovered locations"
    )
    ...
    enemies: Dict[str, Any] = Field(
        default_factory=dict, description="Enemies defeated"
    )
