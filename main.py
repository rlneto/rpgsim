...
Examples:
  ...
  python main.py help            # Show this help message
    ...
    def _setup_commands(self):
        ...
        start_parser.add_argument('--class', type=str, dest='class_name', help='Character class')
        ...
        dev_parser.add_argument('--character', action='store_true', help='Test character creation')
        dev_parser.add_argument('--world', action='store_true', help='Test world system')
        dev_parser.add_argument('--combat', action='store_true', help='Test combat system')
    ...
    def _handle_start(self, args):
        ...
        try:
            # Import character system from main module
            from core.systems.character import Character, get_all_character_classes
            
            # Basic character creation test
            if args.name and args.class_name:
                character = Character()
                success = character.create_character(args.name, args.class_name)
                ...
                if success:
                    ...
                    print(f"Failed to create character: {args.name} ({args.class_name})")
            else:
    ...
    def _handle_test(self, args):
        ...
        try:
            ...
            if args.system:
                success = run_quality_checks(system_only=args.system, verbose=args.verbose)
            else:
    ...
    def _test_character_creation(self):
        ...
        try:
            ...
            character = Character()
            success = character.create_character("TestCharacter", "Warrior")
            
            if success:
                print("Character created successfully")
                print(f"   Name: {character.name}")
                print(f"   Class: {character.class_type.value if character.class_type else 'None'}")
                print(f"   Level: {character.level}")
                print(f"   HP: {character.hp}/{character.max_hp}")
                print(f"   Gold: {character.gold}")
                print(f"   Abilities: {len(character.abilities)}")
                print(f"   Inventory: {len(character.inventory)} items")
                
                # Test class balance
                print("Testing Class Balance...")
                try:
                    from core.systems.character_utils import validate_class_balance, verify_unique_mechanics, verify_minimum_abilities
                    is_balanced = validate_class_balance()
                    print(f"Classes are balanced: {is_balanced}")
                except ImportError:
                    print("Character utils not yet implemented")
            else:
