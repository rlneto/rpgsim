...
def step_verify_peaceful_mood(context):
    ...
    if hasattr(bdd_ctx, "generated_music"):
        ...
        assert pattern.tempo < 100, (
            f"Peaceful music tempo should be < 100, got {pattern.tempo}"
        )
    else:
...
def step_verify_exploration_key(context):
    ...
    if hasattr(bdd_ctx, "generated_music"):
        ...
        assert key in ["C", "G", "D", "F"], (
            f"Exploration key should be common, got {key}"
        )
    else:
        templates = getattr(bdd_ctx, "music_templates", {})
        peaceful_template = templates.get("peaceful", {})
...
def step_verify_combat_mood(context):
    ...
    if hasattr(bdd_ctx, "generated_combat_music"):
        ...
        assert pattern.tempo >= 120, (
            f"Combat music tempo should be >= 120, got {pattern.tempo}"
        )
...
def step_verify_higher_velocity(context):
    ...
    if hasattr(bdd_ctx, "generated_combat_music") and hasattr(
        bdd_ctx, "generated_music"
    ):
        ...
        assert combat_avg > ambient_avg, (
            f"Combat velocity ({combat_avg}) should be higher than ambient ({ambient_avg})"
        )
...
def step_verify_short_durations(context):
    ...
    if hasattr(bdd_ctx, "generated_combat_music"):
        ...
        short_durations = [
            n.duration for n in notes if hasattr(n, "duration") and n.duration <= 0.5
        ]
        assert len(short_durations) > len(notes) * 0.3, (
            "Combat music should have at least 30% short notes"
        )
...
def step_verify_victory_mood(context):
    ...
    if hasattr(bdd_ctx, "generated_victory_music"):
        ...
        assert 100 <= pattern.tempo <= 140, (
            f"Victory tempo should be celebratory, got {pattern.tempo}"
        )
...
def step_verify_class_style(context):
    ...
    assert hasattr(bdd_ctx, "generated_victory_music"), (
        "Victory music should be generated"
    )
    ...
    if hasattr(bdd_ctx, "generated_victory_music"):
        ...
        if len(notes) >= 3:
            ...
            for i in range(len(notes) - 2):
                if (
                    hasattr(notes[i], "midi_note")
                    and hasattr(notes[i + 1], "midi_note")
                    and hasattr(notes[i + 2], "midi_note")
                ):
                    if (
                        notes[i].midi_note < notes[i + 1].midi_note
                        and notes[i + 1].midi_note <= notes[i + 2].midi_note
                    ):
                        ascending_count += 1

            assert ascending_count > 0, (
                "Victory music should have ascending progressions"
            )
...
def step_verify_triumphant_feel(context):
    ...
    if hasattr(bdd_ctx, "generated_victory_music"):
        ...
        assert len(pattern.notes) >= 8, (
            "Triumphant music should have sufficient complexity"
        )
...
def step_location_name_contains_dungeon(context):
    ...
    bdd_ctx = get_bdd_context(context)
    assert "dungeon" in bdd_ctx.current_location, (
        "Location name should contain 'dungeon'"
    )
...
def step_verify_mysterious_mood(context):
    ...
    if hasattr(bdd_ctx, "generated_dungeon_music"):
        ...
        assert pattern.tempo <= 90, (
            f"Mysterious music tempo should be <= 90, got {pattern.tempo}"
        )
...
def step_verify_lower_velocity(context):
    ...
    if hasattr(bdd_ctx, "generated_dungeon_music"):
        ...
        assert avg_velocity <= 70, (
            f"Dungeon music velocity should be lower, got {avg_velocity}"
        )
...
def step_verify_minor_modal_key(context):
    ...
    if hasattr(bdd_ctx, "generated_dungeon_music"):
        ...
        assert pattern.key in ["A", "D", "E"], (
            f"Dungeon key should be mysterious, got {pattern.key}"
        )
...
def step_have_music_pattern(context):
    ...
    pattern = MusicPattern(notes=test_notes, tempo=120, time_signature=(4, 4), key="C")
...
def step_verify_sequence_description(context):
    ...
    bdd_ctx = get_bdd_context(context)
    prompt = bdd_ctx.llm_prompt
    assert "notes" in prompt.lower() or "sequence" in prompt.lower(), (
        "Prompt should describe sequence"
    )
...
def step_verify_suitable_for_music_gen(context):
    ...
    bdd_ctx = get_bdd_context(context)
    prompt = bdd_ctx.llm_prompt
    assert "tempo" in prompt.lower() and "key" in prompt.lower(), (
        "Prompt should be suitable for music generation"
    )
...
def step_verify_loop_duration(context):
    ...
    if hasattr(bdd_ctx, "forest_loop"):
        ...
        total_duration = sum(
            note.duration for note in pattern.notes if hasattr(note, "duration")
        )
        # Convert beats to seconds (approximate)
        estimated_seconds = (total_duration / pattern.tempo) * 60
        assert 25 <= estimated_seconds <= 35, (
            f"Loop should be ~30 seconds, got ~{estimated_seconds}"
        )
...
def step_verify_forest_mood(context):
    ...
    if hasattr(bdd_ctx, "forest_loop"):
        pattern = bdd_ctx.forest_loop
        assert 80 <= pattern.tempo <= 120, (
            f"Forest music should be moderate tempo, got {pattern.tempo}"
        )
...
def step_generate_magical_fantasy_music(context):
    ...
    bdd_ctx.magical_pattern = generator.generate_theme_music(
        mood=MusicMood.MAGICAL, style=MusicStyle.FANTASY, duration_bars=6
    )
...
def step_verify_mystical_tempo(context):
    ...
    if hasattr(bdd_ctx, "magical_pattern"):
        pattern = bdd_ctx.magical_pattern
        assert 80 <= pattern.tempo <= 120, (
            f"Mystical tempo should be 80-120 BPM, got {pattern.tempo}"
        )
...
def step_verify_dynamic_velocity(context):
    ...
    if hasattr(bdd_ctx, "magical_pattern"):
        ...
        if notes:
            ...
            assert velocity_range > 20, (
                f"Velocity should vary dynamically, range: {velocity_range}"
            )
...
def step_verify_enchanting_pattern(context):
    ...
    if hasattr(bdd_ctx, "magical_pattern"):
        ...
        assert len(set(durations)) >= 2, (
            "Should have varied note durations for enchanting feel"
        )
...
def step_verify_victory_mood(context):
    ...
    assert pattern.tempo >= 100, (
        f"Victory music should be upbeat, got {pattern.tempo} BPM"
    )
...
def step_verify_developer_culture(context):
    ...
    assert pattern.tempo >= 110, (
        f"Developer music should be modern tempo, got {pattern.tempo}"
    )
...
def step_verify_loopable_pattern(context):
    ...
    if hasattr(bdd_ctx, "forest_loop"):
        ...
        if len(notes) >= 2:
            ...
            assert abs(last_note.midi_note - first_note.midi_note) <= 12, (
                "Notes should be in loopable range"
            )
...
def step_verify_smooth_transition(context):
    ...
    if hasattr(bdd_ctx, "forest_loop"):
        ...
        if len(notes) >= 4:
            ...
            assert sum(durations) <= 4.0, (
                "Final phrase should resolve within reasonable time"
            )
...
def step_have_music_pattern(context):
    ...
    bdd_ctx.test_pattern = MusicPattern(
        notes=test_notes, tempo=120, time_signature=(4, 4), key="C"
    )
...
def step_verify_file_created(context):
    ...
    assert os.path.exists(bdd_ctx.midi_filename), (
        f"MIDI file {bdd_ctx.midi_filename} should exist"
    )
...
def step_verify_tempo_in_file(context):
    ...
    try:
        ...
        assert midi_data["tempo"] == bdd_ctx.test_pattern.tempo, (
            f"Tempo should match pattern tempo"
        )
    except Exception as e:
...
def step_verify_root_note(context):
    ...
    assert root_found, (
        f"Chord should contain root note C, got: {[n[0] for n in chord_notes]}"
    )
...
def step_verify_third_interval(context):
    ...
    assert third_found, (
        f"Chord should contain third interval E, got: {[n[0] for n in chord_notes]}"
    )
...
def step_verify_fifth_interval(context):
    ...
    assert fifth_found, (
        f"Chord should contain fifth interval G, got: {[n[0] for n in chord_notes]}"
    )
...
def step_verify_midi_values(context):
    ...
    for note_tuple in chord_notes:
        ...
        if note_name in expected_midi:
            ...
            assert abs(midi_value - expected) % 12 == 0, (
                f"MIDI value for {note_name} should be correct class"
            )
...
def step_verify_peaceful_default(context):
    ...
    assert pattern.tempo <= 100, (
        f"Unknown location music should be peaceful tempo, got {pattern.tempo}"
    )
...
def step_verify_universal_applicable(context):
    ...
    assert 50 <= avg_velocity <= 80, (
        f"Should have moderate volume for universal use: {avg_velocity}"
    )
...
def step_verify_moderate_tempo(context):
    ...
    bdd_ctx = get_bdd_context(context)
    pattern = bdd_ctx.unknown_music
    assert 70 <= pattern.tempo <= 110, (
        f"Tempo should be moderate (70-110), got {pattern.tempo}"
    )
...
def step_verify_no_scene_clash(context):
    ...
    if notes:
        ...
        assert max_duration / min_duration <= 8, (
            f"Duration variation should be moderate: {min_duration}-{max_duration}"
        )
