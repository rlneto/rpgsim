...
from services.game_data import SPELL_SCHOOLS_CONFIG, STATUS_EFFECTS_CONFIG
...
class SpellCastingService:
    ...
    def calculate_spell_duration(self, spell_name: str, caster_level: int = 1) -> int:
        ...
        base_duration = spell.duration
        power_scaling = spell.power // 20
        level_bonus = (caster_level - 1) // 3

        school_bonus = 0
        if spell.school.value == "light":
            school_bonus = 1
        elif spell.school.value == "dark":
            school_bonus = 2
    ...
    def _check_spell_requirements(self, caster, spell):
        ...
        if spell.class_requirement and isinstance(caster, Character) and caster.class_type != spell.class_requirement:
            return False
    ...
    def _apply_status_effect(self, target, status_effect, spell_power, caster):
        ...
        effect_config = STATUS_EFFECTS_CONFIG.get(status_effect)
        if not effect_config:
            return False

        if isinstance(target, Character):
            immune_classes = effect_config.get("immune_classes", [])
            if target.class_type in immune_classes:
                return False
    ...
    def _calculate_spell_damage(self, caster, target, spell):
        ...
        school_config = SPELL_SCHOOLS_CONFIG.get(spell.school.value, {})
    ...
    def _calculate_spell_hit_chance(self, caster, target, spell_school):
        school_config = SPELL_SCHOOLS_CONFIG.get(spell_school, {})
